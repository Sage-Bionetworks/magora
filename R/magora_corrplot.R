...
#'
#' Creates a correlation plot comparing nanostring data to human genetic data, faceted by age. Combines multiple plots (created via \link{single_age_corrplot}), one for each age of the mice in the sample.
#'
...
#' @examples {
#' nanostring %>%
#'   dplyr::filter(age_months == "4 Months") %>%
#'   single_age_corrplot()
#' }
single_age_corrplot <- function(data) {
  ggplot2::ggplot(data) +
    ...
    ggplot2::facet_grid(cols = dplyr::vars(age_months)) +
