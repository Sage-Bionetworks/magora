magora_volcano_plotly <- function(data, pvalue = pvalue, log_fc_cutoff = 1, pvalue_cutoff = 0.05) {
  ...
  p <- ggplot2::ggplot(data, ggplot2::aes(x = log2fold_change, y = -log10({{pvalue}}), colour = diff_expressed, text = gene)) +
