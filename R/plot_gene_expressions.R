...
#'
#' @param data Input data (\code{\link{gene_expressions}} filtered by gene, mouse line, and tissue).
#'
#' @return A ggplot2 object.
#' @export
plot_gene_expressions <- function(data) {
  ...
  measured_annotation <- data %>%
    ...
    dplyr::mutate(label = "This gene cannot be\nmeasured in this mouse line.")

  p <- ggplot2::ggplot(data) +
    ggplot2::facet_wrap(ggplot2::vars(.data$mouse_line), ncol = 2, drop = FALSE) +
    ggplot2::geom_boxplot(ggplot2::aes(x = .data$age, y = .data$tpm, fill = paste("Sex:", .data$sex)), position = ggplot2::position_dodge2(preserve = "single"))
  ...
  p +
    ...
    ggplot2::labs(x = "Age", y = "Transcripts Per Million (TPM)") +
